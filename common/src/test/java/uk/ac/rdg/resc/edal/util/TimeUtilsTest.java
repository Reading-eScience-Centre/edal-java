...
public class TimeUtilsTest {
	...
	public void testgetTimeStringForCapabilities() {
		...
		String expected =TimeUtils.dateTimeToISO8601(start) +"/"
						+TimeUtils.dateTimeToISO8601(lastDate)
						+"/" +oneDayPeriod;
		...
		expected = expected + ","+
		           TimeUtils.dateTimeToISO8601(secondPeriod.get(0)) +"/" +
	...
	@Test
	public void testGetTimeRangeForString(){
		String timeString ="1990/12/30;2000/01/01";
		try{
			assertEquals("", TimeUtils.getTimeRangeForString(timeString, 
                                                           AllLeapChronology.getInstanceUTC()));
