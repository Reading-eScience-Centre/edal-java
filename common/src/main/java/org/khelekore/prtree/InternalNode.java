...
class InternalNode<T> extends NodeBase<Node<T>, T> {
    ...
    public void expand (MBR mbr, NodeFilter<T> filter,
			MBRConverter<T> converter, List<T> found,
    ...
    public void find (MBR mbr, MBRConverter<T> converter,
		      List<T> result, NodeFilter<T> filter) {
	for (int i = 0, s = size (); i < s; i++) {
	    Node<T> n = get (i);
	    if (mbr.intersects (n.getMBR (converter)))
