...
public class SLDSubsampledGlyphSymbolizer implements SLDSymbolizer {

	private String layerName;
	private Node symbolizerNode;
	private ImageLayer imageLayer;
	
	public SLDSubsampledGlyphSymbolizer(String layerName, Node symbolizerNode) throws SLDException {
		try {
	...
	private ImageLayer parseSymbolizer() throws XPathExpressionException,
		...
		String glyphSpacingText = (String) xPath.evaluate(
				"./resc:GlyphSpacing", symbolizerNode, XPathConstants.STRING);
		Float glyphSpacing = 1.5F;
		if (!(glyphSpacingText == null) && ! (glyphSpacingText.equals(""))) {
			glyphSpacing = Float.parseFloat(glyphSpacingText);
		}
